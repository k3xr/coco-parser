...
public class Scanner {
	...
	static {
		...
		for (int i = 97; i <= 99; ++i) start.set(i, 1);
		for (int i = 101; i <= 122; ++i) start.set(i, 1);
		for (int i = 48; i <= 57; ++i) start.set(i, 2);
		for (int i = 13; i <= 13; ++i) start.set(i, 4);
		start.set(34, 3); 
		start.set(59, 6); 
		start.set(100, 24); 
		start.set(40, 13); 
		...
		literals.put("new", new Integer(14));
		literals.put("Array", new Integer(15));
		literals.put("function", new Integer(23));
		literals.put("return", new Integer(24));
		literals.put("if", new Integer(25));
		literals.put("while", new Integer(26));
	...
	Token NextToken() {
		...
		loop: for (;;) {
			switch (state) {
				...
				case 3:
					if (ch <= 9 || ch >= 11 && ch <= 12 || ch >= 14 && ch <= '!' || ch >= '#' && ch <= 147 || ch >= 149 && ch <= 65535) {AddCh(); state = 3; break;}
				...
				case 6:
					{t.kind = 6; break loop;}
				case 7:
					if (ch == 'w') {AddCh(); state = 8; break;}
					else {state = 0; break;}
				case 8:
					if (ch == 'r') {AddCh(); state = 9; break;}
					else {state = 0; break;}
				case 9:
					if (ch == 'i') {AddCh(); state = 10; break;}
					else {state = 0; break;}
				case 10:
					if (ch == 't') {AddCh(); state = 11; break;}
					else {state = 0; break;}
				case 11:
				...
				case 24:
					...
					else if (ch == 'o') {AddCh(); state = 26; break;}
					else {t.kind = 1; t.val = new String(tval, 0, tlen); CheckLiteral(); return t;}
				case 25:
